...
public class Zipper {
    ...
    /*@
    	...
    	requires s1.length() == s2.length();
    */
    public static String zip2(String s1, String s2) throws TooFewArgumentsException, ArgumentsLengthDifferException{
    	if(s1 == null || s2 == null){
    		throw new TooFewArgumentsException();
    	}else if(s1.length()!=s2.length()){
    		throw new ArgumentsLengthDifferException();
    	}else{
    		String result = "";
        	for (int i = 0; i < s1.length(); i++) {
        		result += Character.toString(s1.charAt(i)) + Character.toString(s2.charAt(i));
...
//        } else if (s1.length() != s2.length()) {
//            System.out.println("error: length of command line arguments differs ("
//                    + s1.length() + ", " + s2.length() + ")");
//        } else {
