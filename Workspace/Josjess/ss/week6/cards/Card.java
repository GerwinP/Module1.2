...
public class Card implements Serializable{
	...
	private static final String[] RANK_STRINGS = { "2", "3", "4", "5", "6", "7", "8", "9", "10", "jack", "queen", "king", "ace" };
	...
	public static Card read(BufferedReader in) throws IOException {
		...
		if (nextLine != null) {
			Scanner delimiter = new Scanner(nextLine).useDelimiter("\\s+");
			String suit = delimiter.next();
			String rank = delimiter.next();
			if (Arrays.asList(RANK_STRINGS).contains(rank) && Arrays.asList(SUIT_STRINGS).contains(suit)) {
				Card card = new Card(suitString2Char(suit), rankString2Char(rank));
				return card;
			} else {
	...
	public static Card read(ObjectInputStream ois) throws EOFException{
		Card card = null;
		try {
