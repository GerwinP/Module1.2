...
public class Hotel {
	...
	public Hotel(String name){
		...
		room1 = new PricedRoom(101, 35, 0.35);
	...
	public Bill getBill(String nameGuest, int number, PrintStream theOutStream){
		if (nameGuest == null || !(this.getRoom(nameGuest) instanceof PricedRoom)){
			return null;
		}
		Bill bill = new Bill(theOutStream);
		for(int i = 1;i<=number;i++){
			bill.newItem((PricedRoom) this.getRoom(nameGuest));
		}
		bill.close();
		return bill;
	}
	...
	public static void main(String[] args){
		Hotel hotel = new Hotel("IA");
		hotel.checkIn(Password.INITIAL, "Sien");
		hotel.getBill("Sien", 11, System.out);
